...
regime_change=function(delP,annP,delT){
  
#regime_change=function(delP,delT) 

dir=("E:\\USU_Research_work\\Sensitivity_test\\B21_TOPNETRUN")
setwd(dir)
...
# ###changing  only summer precipitation #####################
n=seq(1,33,1)
x1=365*n-215
x2=365*n-95
dates=seq(as.Date("1980/1/1"), as.Date("2012/12/31"), "day")
rainmonth=matrix(colMeans(data.frame(monthlyfunction(rain1, FUN=sum, na.rm=TRUE,dates=dates))/33))
rainann=matrix(rowMeans(data.frame(annualfunction(rain1, FUN=sum, na.rm=TRUE,dates=dates))/33))
summer_rain=(rainmonth[c(7,8,9)])
rain_remian=rainmonth[c(1,2,3,4,5,6,10,11,12)]

#s1_rain=summer_rain*delP
#s2_rain=rainann-sum(s1_rain)
#delP_othermonth=matrix(s2_rain/sum(rain_remian))

delp_summer=delP;delP_othermonth=annP
delp_all=rep(delP_othermonth,12054*24)
delp_all=matrix(delp_all,nrow=12054,ncol=24)



for (i in 1 :33){
delp_all[x1[i]:x2[i],]=delp_summer
}


rain_summer2=rain1*delp_all
#precip=data.frame(rain2)
precip=data.frame(rain_summer2)
...
rg=mapply(regime_change,df$x,df$y,df$z)
