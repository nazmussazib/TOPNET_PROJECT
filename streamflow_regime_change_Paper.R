...
x=c(20,4,150,3.5,200,55,170,0.45,0.4,0.15,6,1,150,-0.1,1.6,1)
y=c(74,7.55,350,8,470,135,240,0.74,0.73,0.35,20,4,320,0.37,19,0.75)
...
##for priting purpose plase see avbe figure how to print 

##ggplot(data=myData) +
  ##geom_bar(aes(x=Watershed, y=value, fill=Z), stat="identity") +scale_fill_manual(values = rhg_cols1)+
print(ggplot(data=myData, aes(x = Watershed, y = value)) + 
  geom_bar(aes(fill = Z), stat="identity", position = "dodge")+scale_fill_manual(values = rhg_cols1)+
...
ggsave("regime_changes_paper.tiff", width=14, height=12, dpi=500)
dev.off()
