...
calibration=function(x){
  ...
  cat(sprintf("%d  %d  %s",2191,1827, "nsteps  (11325 is 31 years, 19591001 to 19901002.  16894 is 46 years 19591001 to 20051231)"),file='topinp.dat',append=TRUE); cat("\n", file="topinp.dat", append=TRUE)
  ...
  time_cali= seq(as.Date("1999/1/1"), as.Date("2002/12/31"), "day") # create time series based on start and end date
  ...
  calibrated_flow=data.frame(simu=simu_flow[731:2191],observ=observd_flow)
  plot(time_cali,calibrated_flow$observ, type="o", col="blue",,xlab=" Time days",ylab="stream flow (m3/s)")
  lines(time_cali,calibrated_flow$simu, type="o", pch=22, lty=2, col="red")
...
#library(hydroPSO)
#xe1=(c(1,1,0.5,0.5,0.5,0.1,0.75,1))
#xe2=(c(1000,1000,1,2,3,10,10,1000))
#hydroPSO(fn=calibration,lower=xe1,upper=xe2,control=list(MinMax='max'))
#xd=cbind(xe1,xe2)
...
xe1=(c(1,1,0.5,0.5,0.5,0.1,0.75,1))
xe2=(c(1000,1000,1,2,3,10,10,1000))
calibration_result=nsga2(calibration,8,2,generations=150, popsize=100,lower.bounds=c(1,1,0.5,0.5,0.5,0.1,0.75,1),upper.bounds=c(1000,1000,1,2,3,10,10,1000))
...
text(time_c[1100],135, "C21",cex = 2)
#text(time_c[150],610, "Calibration",cex = 1.25)
text(time_c[400],120, paste("Bias=",format(Bias_c,digit=2)),cex = 1.25)
text(time_c[400],110, paste("NSE=",format(CE_c,digit=2)),cex = 1.25)
text(time_c[1950],120, paste("Bias=",format(Bias_v,digit=2)),cex = 1.25)
text(time_c[1950],110, paste("NSE=",format(CE_v,digit=2)),cex = 1.25)
...
plot(time_c,cumsum(0.23*cv_flow$observ[1:1096]),col='blue',ylab="cumulative flow (mm)")
lines(time_c,cumsum(0.23*cv_flow$simu[1:1096]),col='red')

plot(time_c,cumsum(simu_rain[731:1826]),ylab="cumulative rainfall/flow (mm)")
lines(time_c,cumsum(0.23*cv_flow$observ[1:1096]),col='blue',ylab="cumulative flow (mm)")
lines(time_c,cumsum(0.23*cv_flow$simu[1:1096]),col='red')
...
text(15100,162, paste("Bias=",format(Bias_v,digit=2)),cex = 1.25)
text(15100,147, paste("NSE=",format(CE_v,digit=2)),cex = 1.25)
text(15100,132, paste("MSE=",format(Mean_error_v,digit=2)),cex =1.25)



plot(time_cv,cumsum(0.23*cv_flow$observ),col='blue',ylab="cumulative flow (mm)")
lines(time_cv,cumsum(0.23*cv_flow$simu),col='red')
...
lines(time_cv,cumsum(0.23*cv_flow$observ),col='blue',ylab="cumulative flow (mm)")
lines(time_cv,cumsum(0.23*cv_flow$simu),col='red')
legend(14650, 800, c("rainfall","observed flow","simulated flow"), cex=0.8, col=c("black","blue","red"), pch=21, lty=1);
...
plot(time, calibrated_flow$simu, axes=T, ylim=c(0,250), xlab="", ylab="",type="l",lty=1,col="red", main="")
points(time, calibrated_flow$simu,pch=20,col="red")
axis(2, ylim=c(0,30),col="red",lwd=2)
mtext(2,text="Stream flow(m3/s)",line=2)

par(new=T)
plot(time, calibrated_flow$observ, axes=T, ylim=c(0,250), xlab="", ylab="",type="l",lty=2,col="blue", main="")
points(time, calibrated_flow$observ,pch=20,col="blue")
axis(2, ylim=c(0,600),col="blue",lwd=2)
#mtext(2,text="NSE",line=2)

par(new=T)
plot(time, simu_rain[732:1827,], axes=F, ylim=rev(c(0,450)), col="darkgrey",xlab="", ylab="", type="l",lty=3, main="",lwd=2)
