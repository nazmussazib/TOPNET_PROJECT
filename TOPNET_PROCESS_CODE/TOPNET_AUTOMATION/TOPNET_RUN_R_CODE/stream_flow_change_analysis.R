...
setwd("E:\\USU_Research_work\\TOPNET PROJECT\\MODEL COMPARISON\\A1_watershed_final\\A1_TOPNETRUN_HIS") #for A1

##RUN MODEL FOR EACH RCP AND SAVED IT DIFFERENT DIRECTORY
##RCP26_65, RCP26_99 etc

setwd("E:\\USU_Research_work\\TOPNET PROJECT\\MODEL COMPARISON\\A1_watershed_final\\A1_TOPNETRUN") #for A1
...
obs=data.frame(obs_flow[1:9497])
date_base=seq(as.Date("1980/1/1"), as.Date("2005/12/31"), "day")

obs_base<- data.frame(monthlyfunction(obs, FUN=mean, na.rm=TRUE,dates=date_base))
sf_base1<- data.frame(monthlyfunction(simu_flow, FUN=mean, na.rm=TRUE,dates=date_base))


plot(seq(1,12,1),sf_base1)
lines(seq(1,12,1),obs_base)
lines(seq(1,12,1),sf_base1,col='red') ##canEsm2
lines(seq(1,12,1),sf_base1,col='green') ##CCSM4
lines(seq(1,12,1),sf_base1,col='blue') ##MIROC5
lines(seq(1,12,1),sf_base1,col='yellow') ##MIROC5
lines(seq(1,12,1),sf_base1,col='grey',lty=2) ##MIROC5
