...
origindir <- c("E:/USU_Research_work/TOPNET PROJECT/MODEL COMPARISON/B1_Watershed/B1_WD")
targetdir <- c("E:/USU_Research_work/TOPNET PROJECT/MODEL COMPARISON/B1_Watershed/B1_TOPNETRUN")
...
origindir <- c("E:/USU_Research_work/TOPNET PROJECT/MODEL COMPARISON/B1_Watershed/B1_SD")
...
origindir <- c("E:/USU_Research_work/TOPNET PROJECT/MODEL COMPARISON/B1_Watershed/B1_CD/B1_CD_Present/B1_CD_Daymet")
#targetdir <- c("E:/USU_Research_work/TOPNET PROJECT/INPUT_AUTOMATION/LittleBearRiverDEMO/DEMO_test_1/TOPNET_RUN")
...
setwd("E:/USU_Research_work/TOPNET PROJECT/MODEL COMPARISON/B1_Watershed_final/B1_TOPNETRUN")
...
lines(rep(time_c[1461],84)  ,seq(0,83,1),col="grey60",lty=2)
...
text(time_c[1100],80, "B1",cex = 2)
#text(time_c[150],610, "Calibration",cex = 1.25)
text(time_c[400],75, paste("Bias=",format(Bias_c,digit=2)),cex = 1.25)
text(time_c[400],70, paste("NSE=",format(CE_c,digit=2)),cex = 1.25)
text(time_c[1950],75, paste("Bias=",format(Bias_v,digit=2)),cex = 1.25)
text(time_c[1950],70, paste("NSE=",format(CE_v,digit=2)),cex = 1.25)
...
plot(time_c,cumsum(0.45*cv_flow$observ[1:1096]),col='blue',ylab="cumulative flow (mm)")
lines(time_c,cumsum(0.45*cv_flow$simu[1:1096]),col='red')

plot(time_c,cumsum(simu_rain[732:1827]),ylab="cumulative rainfall/flow (mm)")
lines(time_c,cumsum(0.45*cv_flow$observ[1:1096]),col='blue',ylab="cumulative flow (mm)")
lines(time_c,cumsum(0.45*cv_flow$simu[1:1096]),col='red')
...
text(15400,40, paste("Bias=",format(Bias_v,digit=2)),cex = 1.25)
text(15400,36, paste("NSE=",format(CE_v,digit=2)),cex = 1.25)
text(15400,32, paste("MSE=",format(Mean_error_v,digit=2)),cex =1.25)

plot(time_cv,cumsum(0.45*cv_flow$observ),col='blue',ylab="cumulative flow (mm)")
lines(time_cv,cumsum(0.45*cv_flow$simu),col='red')
...
lines(time_cv,cumsum(0.45*cv_flow$observ),col='blue',ylab="cumulative flow (mm)")
lines(time_cv,cumsum(0.45*cv_flow$simu),col='red')
legend(14650, 800, c("rainfall","observed flow","simulated flow"), cex=0.8, col=c("black","blue","red"), pch=21, lty=1);
...
time_all= seq(as.Date("1980/1/1"), as.Date("2012/12/31"), "day") #create time series
time_cali= seq(as.Date("2011/1/1"), as.Date("2012/12/31"), "day") # create time series based on start and end date
date_overlap=match(time_cali,time_all,) # get overlap time interval
observd_flow=matrix(obs_flow[date_overlap])
observd_flow[observd_flow<0] <- NA
calibrated_flow=data.frame(simu=simu_flow[1827:2557],observ=observd_flow) ##take only from 2010/01/01---2012/12/31
time= seq(as.Date("2011/1/1"), as.Date("2012/12/31"), "day") # create time series based on start and end date
...
plot(time, calibrated_flow$simu, axes=T, ylim=c(0,80), xlab="", ylab="",type="l",lty=1,col="red", main="")
points(time, calibrated_flow$simu,pch=20,col="red")
axis(2, ylim=c(0,30),col="red",lwd=2)
mtext(2,text="Stream flow(m3/s)",line=2)

par(new=T)
plot(time, calibrated_flow$observ, axes=T, ylim=c(0,80), xlab="", ylab="",type="l",lty=2,col="blue", main="")
