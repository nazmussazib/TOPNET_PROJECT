...
origindir <- c("E:/USU_Research_work/TOPNET PROJECT/MODEL COMPARISON/B21_Watershed/B21_WD")
targetdir <- c("E:/USU_Research_work/TOPNET PROJECT/MODEL COMPARISON/B21_Watershed/B21_TOPNETRUN")
...
origindir <- c("E:/USU_Research_work/TOPNET PROJECT/MODEL COMPARISON/B21_Watershed/B21_SD")
...
origindir <- c("E:/USU_Research_work/TOPNET PROJECT/MODEL COMPARISON/B21_Watershed/B21_CD/B21_CD_present")
#targetdir <- c("E:/USU_Research_work/TOPNET PROJECT/INPUT_AUTOMATION/LittleBearRiverDEMO/DEMO_test_1/TOPNET_RUN")
...
calibration=function(x){
  ...
  time_cali= seq(as.Date("2004/1/1"), as.Date("2005/12/31"), "day") # create time series based on start and end date
  ...
  calibrated_flow=data.frame(simu=simu_flow[366:1096],observ=observd_flow)
  plot(observd_flow, type="o", col="blue",,xlab=" Time days",ylab="stream flow (m3/s)")
  lines(simu_flow[366:1096], type="o", pch=22, lty=2, col="red")
  
  legend(120, max(observd_flow-15), c("observed flow","simulated flow"), cex=0.8, col=c("blue","red"), pch=21:22, lty=1:2);
  res=NSE(calibrated_flow$simu,calibrated_flow$observ)
  bias=sum(calibrated_flow$simu)/sum(calibrated_flow$observ)
  return(res)
}
