...
c      SUBROUTINE INPUTT (initt, iend, neq, qact, actime, modelid,
c     &                   npar, nrx, iex)
c	!MS$ATTRIBUTES DLLEXPORT :: inputt

c  DGT 5/27/12  DGT making this main program to attempt to resolve model running problem
c  following lines are from top_main_v7.f
c   Main program to drive topmodel in standalone mode
      program top1
 	USE Data_Read   ! DGT 5/27/12 moved from below 	
...
!  End of code from top_main_v7.f

!	USE Data_Read  moved to top
...
c
!      integer*4  nrx, iex
c      integer*4 initt(iex), iend(iex), neq, i, j, npar, 
      integer*4 k
!      REAL*8 qact(nrx,iex), actime(nrx) 
!      character*30 modelid
	LOGICAL*4 limitc
!  END INPUTT Declarations
! BEGIN MODEL Declarations
...
c   outputs
      REAL*8 Q4(MAXINT,MAXCHN),qmod,mse,mae    
C RPI 14/8/2003 remove Q5
c	REAL*8 Q5(MAXINT+1,MAXCHN)  ! RPI 31/7/2002 for neq=0 + MAXCHN 
      LOGICAL*1 OK
!	INTEGER M, MI, MPS, MPE
!	INTEGER IGAPS(3),
	integer ntdh(MAXSLP),js   !RAW changed maxint to max slp! RPI 5/8/02 dimensioned ntdh
...
c RPI 9/6/99 added the following line to define STIM
      ...
      CALL TD8_MICSEC(SDATE,SHOUR,STIM8)	! RPI 9/2/2002 changed stim to stim8
	STIM=STIM8    
c	neq = 1
c	initt(neq) = 1
c	iend(neq) = M
...
c Deallocate dynamic arrays
	...
	DEALLOCATE (LL,NTR,llout,LINKR,clinkr,RP,RPd,qmap)
      ...
      go to 1234  ! DGT 5/27/12 rather than return go to where code after INPUTT call was insterted
      ! return
...
!      stop
  ...
  208   FORMAT(' ***** Error in, or no RAIN.DAT')
	...
	!  The code following was the executable code between INPUTT and MODEL calls in top_main_v7.f
 1234	continue
 	IF(neq .gt. iex)
	...
	ENDDO
      ...
      ibeale=0
	! end of code from top_main_v7.f before CALL MODEL
...
c**************************************************************************
c      SUBROUTINE MODEL (it, iflag, iopt, prt, neq, npar, npx, iex, 
c     *                  nfor, qfit, dfit, par, mfit, ifit, ibeale)
c	!MS$ATTRIBUTES DLLEXPORT :: model
C
C         THE MODEL
c      USE Data_Read
c      IMPLICIT NONE ! 30/03/2004 ITB 


c      INCLUDE 'tdims_v7.INC'
c      INCLUDE 'LUNS.INC'
...
!      LOGICAL*1 OK
!	INTEGER M, MI, MPS, MPE
!	INTEGER IGAPS(3),ntdh(MAXSLP),js   !RAW changed maxint to max slp! RPI 5/8/02 dimensioned ntdh
!	INTEGER*4 SDATE, SHOUR, INT
!C
!	INTEGER*4 STIM
!cmmmm	INTEGER*4,ALLOCATABLE :: times(:)
