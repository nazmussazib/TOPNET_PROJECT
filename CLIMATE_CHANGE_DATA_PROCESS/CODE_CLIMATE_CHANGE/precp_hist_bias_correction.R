...
Mod=c('CCSM4','CanESM2','MRI-CGCM3','CSIRO-Mk3-6-0')
Model=Mod[1]
...
dym_without_leap=his_dym[-lp_id,]
huss_mat=matrix(NA,nrow=length(dates_bc),ncol=1)
huss_mat[lp_id]=-999
huss_mat[is.na(huss_mat)]=shumd[,1]

##for psl
dates_psl=seq(as.Date("1950/1/2"), as.Date("2005/12/31"), "day") 
...
huss_mod_his=data.frame(huss_mat)
...
u_wind <- NCEP.gather(variable='uwnd.sig995',level='surface',months.minmax=c(1,12), years.minmax=c(1980,2005),lat.southnorth=c(41.8,41.48), lon.westeast=c(360-117.6,360-117.25),reanalysis2 = FALSE, return.units = TRUE)
v_wind <- NCEP.gather(variable='vwnd.sig995',level='surface',months.minmax=c(1,12), years.minmax=c(1980,2005),lat.southnorth=c(41.8,41.48), lon.westeast=c(360-117.6,360-117.25),reanalysis2 = FALSE, return.units = TRUE)
...
wind_data_h=t(matrix(wind_six_hours$wind,nrow=24,ncol=nrow(wind_six_hours)/24))

wind_data_Daily=rowMeans(wind_data_h)
wind=data.frame(wind_data_Daily)
wind <- lapply(wind, function(.col){ if (is.numeric(.col)) return(sprintf("%8.2f",.col))else return(.col)}) 

wind_speed=data.frame(wind,day,hour) ##Need to change dss
wind_speed=data.frame(wind,day,hour) ##Need to change dss
