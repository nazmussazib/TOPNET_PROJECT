...
Mod=c('CCSM4','CanESM2','GFDL-CM3','MPI-ESM-MR','MIROC5')
Model=Mod[1]
setwd(paste("F:\\USU_Research_work_update_March_30_2014\\TOPNET PROJECT\\MODEL COMPARISON\\CLIMATE_CHANGE_ALLDATA\\",Model,sep=""))
pr.nc = nc_open(paste("BCCA_0.125deg_tasmax_day_",Model,"_rcp26_r2i1p1_20460101-20551231.nc",sep=""))
...
w=8
setwd(paste("E:\\USU_Research_work\\TOPNET PROJECT\\MODEL COMPARISON\\",watsed[w],"_Watershed_final2\\",watsed[w],"_CC_data\\",Model,sep=""))##for A1:
#setwd("E:\\USU_Research_work\\TOPNET PROJECT\\MODEL COMPARISON\\B21_NC\\B21_CC_data\\MPI-ESM-MR")##for A1:
...
station_monthly<- data.frame(monthlyfunction(avgT_d, FUN=mean, na.rm=TRUE,dates=dates))
station_annual<- t(as.matrix(annualfunction(avgT_d, FUN=mean, na.rm=TRUE,dates=dates)))
sta_ID=seq(1,nc,1)
sta_ele=rep(-999,nc)
...
a=proj_rain1[,4:(len+3)]
b=proj_rain2[,4:(len+3)]
c=proj_rain3[,4:(len+3)]
d=proj_rain4[,4:(len+3)]
...
pr_proj_mon1=data.frame(monthlyfunction(proj_rain1[,4:(len+3)], FUN=sum, na.rm=TRUE,dates=dat_1))/20
pr_proj_mon2=data.frame(monthlyfunction(proj_rain2[,4:(len+3)], FUN=sum, na.rm=TRUE,dates=dat_1))/20
pr_proj_mon3=data.frame(monthlyfunction(proj_rain3[,4:(len+3)], FUN=sum, na.rm=TRUE,dates=dat_2))/20
pr_proj_mon4=data.frame(monthlyfunction(proj_rain4[,4:(len+3)], FUN=sum, na.rm=TRUE,dates=dat_2))/20
...
tmax_proj_mon1=data.frame(monthlyfunction(proj_tmax1[,4:(len+3)], FUN=mean, na.rm=TRUE,dates=dat_1))
tmax_proj_mon2=data.frame(monthlyfunction(proj_tmax2[,4:(len+3)], FUN=mean, na.rm=TRUE,dates=dat_1))
tmax_proj_mon3=data.frame(monthlyfunction(proj_tmax3[,4:(len+3)], FUN=mean, na.rm=TRUE,dates=dat_2))
tmax_proj_mon4=data.frame(monthlyfunction(proj_tmax4[,4:(len+3)], FUN=mean, na.rm=TRUE,dates=dat_2))

tmin_proj_mon1=data.frame(monthlyfunction(proj_tmin1[,4:(len+3)], FUN=mean, na.rm=TRUE,dates=dat_1))
tmin_proj_mon2=data.frame(monthlyfunction(proj_tmin2[,4:(len+3)], FUN=mean, na.rm=TRUE,dates=dat_1))
tmin_proj_mon3=data.frame(monthlyfunction(proj_tmin3[,4:(len+3)], FUN=mean, na.rm=TRUE,dates=dat_2))
tmin_proj_mon4=data.frame(monthlyfunction(proj_tmin4[,4:(len+3)], FUN=mean, na.rm=TRUE,dates=dat_2))
...
tmin_proj_mon1=data.frame(monthlyfunction(tmin_proj1[,4:(len+3)], FUN=mean, na.rm=TRUE,dates=dat_1))
tmin_proj_mon2=data.frame(monthlyfunction(tmin_proj2[,4:(len+3)], FUN=mean, na.rm=TRUE,dates=dat_1))
tmin_proj_mon3=data.frame(monthlyfunction(tmin_proj3[,4:(len+3)], FUN=mean, na.rm=TRUE,dates=dat_2))
tmin_proj_mon4=data.frame(monthlyfunction(tmin_proj4[,4:(len+3)], FUN=mean, na.rm=TRUE,dates=dat_2))
...
##finding leap year
dates_win_his=seq(as.Date("1980/1/1"), as.Date("2009/12/31"), "day") 
dates_win_his=dates_bc
...
wind_mod_his=ws_his[,1] ##only for MPI-ESM-MR
...
setwd(paste("E:\\USU_Research_work\\TOPNET PROJECT\\MODEL COMPARISON\\",watsed[w],"_watershed_final2\\",watsed[w],"_CC_data\\",Model,sep=""))

setwd("E:\\USU_Research_work\\TOPNET PROJECT\\MODEL COMPARISON\\B21_NC\\B21_CC_data\\MPI-ESM-MR")##for A1:

wind_obs_his=read.table('wind_obs.dat',skip=3)
wind_obs_his=wind_obs_his[1:9497,]
...
strDates_wind_his=data.frame(date=dates_win_his)
#strDates_wind_his=rbind(data.frame(date=dat_1),data.frame(date=dat_2))
gh_wind_his=gsub("-","", strDates_wind_his$date, fixed=TRUE)
day_wind_his=data.frame(time=gh_wind_his)
hour_wind_his=rep.int(240000, nrow(day_wind_his))
