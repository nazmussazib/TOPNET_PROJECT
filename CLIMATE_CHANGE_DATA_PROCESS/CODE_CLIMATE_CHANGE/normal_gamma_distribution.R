...
for(i in 1:(len)){
  ...
  for ( j in 1:12){
    ...
    proj_corr2 <- doQmapRQUANT(proj1,rain_qm.fit,type="tricub")
    proj_corr4 <- doQmapRQUANT(proj2,rain_qm.fit,type="tricub")
    proj_corr6 <- doQmapRQUANT(proj3,rain_qm.fit,type="tricub")
    proj_corr8 <- doQmapRQUANT(proj4,rain_qm.fit,type="tricub")
...
for(i in 1:len){
  ...
  for ( j in 1:12){
    ...
    obs=(tmaxobs[id_his,i]);
    p1=pnorm(obs,mean(obs),sd(obs));
    sim=(tmaxsim[id_his,i]);
    p2=pnorm(sim,mean(sim),sd(sim));
   
    
    his_corr=qnorm(p2,mean(obs),sd(obs))
    
    proj1=(proj_tmax1[id_proj1,i+3]);proj_corr1=pnorm(proj1,mean(sim),sd(sim));proj_corr2=qnorm(proj_corr1,mean(obs),sd(obs));
    proj2=(proj_tmax2[id_proj1,i+3]);proj_corr3=pnorm(proj2,mean(sim),sd(sim));proj_corr4=qnorm(proj_corr3,mean(obs),sd(obs));
    proj3=(proj_tmax3[id_proj2,i+3]);proj_corr5=pnorm(proj3,mean(sim),sd(sim));proj_corr6=qnorm(proj_corr5,mean(obs),sd(obs));
    proj4=(proj_tmax4[id_proj2,i+3]);proj_corr7=pnorm(proj4,mean(sim),sd(sim));proj_corr8=qnorm(proj_corr7,mean(obs),sd(obs));
    ...
    ps=startberngamma(obsdr)
    ts=startberngamma(simdr)
    
    p2dr=pberngamma(simdr,prob=ts$prob,scale=ts$scale,shape=ts$shape);his_corrdr=qberngamma(p2dr,prob=ps$prob,scale=ps$scale,shape=ps$shape)
    
    
    
    proj1dr=(proj_dtr1[id_proj1,i+3]);proj_corr1dr=pberngamma(proj1dr,prob=ts$prob,scale=ts$scale,shape=ts$shape);proj_corr2dr=qberngamma(proj_corr1dr,prob=ps$prob,scale=ps$scale,shape=ps$shape);
    proj2dr=(proj_dtr2[id_proj1,i+3]);proj_corr3dr=pberngamma(proj2dr,prob=ts$prob,scale=ts$scale,shape=ts$shape);proj_corr4dr=qberngamma(proj_corr3dr,prob=ps$prob,scale=ps$scale,shape=ps$shape);
    proj3dr=(proj_dtr3[id_proj2,i+3]);proj_corr5dr=pberngamma(proj3dr,prob=ts$prob,scale=ts$scale,shape=ts$shape);proj_corr6dr=qberngamma(proj_corr5dr,prob=ps$prob,scale=ps$scale,shape=ps$shape);
    proj4dr=(proj_dtr4[id_proj2,i+3]);proj_corr7dr=pberngamma(proj4dr,prob=ts$prob,scale=ts$scale,shape=ts$shape);proj_corr8dr=qberngamma(proj_corr7dr,prob=ps$prob,scale=ps$scale,shape=ps$shape);
