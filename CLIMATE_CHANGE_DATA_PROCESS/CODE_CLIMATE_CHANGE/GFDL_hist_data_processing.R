...
pr.nc = nc_open("BCCA_0.125deg_tasmax_day_GFDL-CM3_historical_r1i1p1_19800101-19841231.nc")
...
hus <- function(file ){
  ...
  hus.nc = nc_open(file)
  hus_time=as.Date(ncvar_get(hus.nc,'time'),origin="1860-01-01")
...
##finding leap year
dates_hus=seq(as.Date("1980/1/1"), as.Date("2005/12/31"), "day") 
...
##for psl
dates_psl=seq(as.Date("1980/1/1"), as.Date("2005/12/31"), "day") 
...
for ( k in 1:len){
  tmaxtmintdew=cbind(tmaxtmintdew,tmax_final[,k],tmin_final[,k],tmin_final[,k])
}
...
##finding leap year
dates_win=seq(as.Date("1980/1/1"), as.Date("2005/12/31"), "day") 
...
plot(wind_mod_his$wind,wind_obs_his$wind)
